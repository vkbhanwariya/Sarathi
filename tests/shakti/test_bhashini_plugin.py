...
class TestBhashiniClient:
    ...
    def test_translate_text_empty_response_raises_execution_failed(self) -> None:
        client = BhashiniClient(user_id="usr_abc", api_key="valid_key", inference_key="inf_key")

        class MockResponse:
            status_code = 200
            text = '{"pipelineResponse": [{"output": []}]}'

        with patch("httpx.Client.post", return_value=MockResponse()):
            with pytest.raises(DoshError) as exc_info:
                client.translate_text("some text", "hi", "en")
            assert exc_info.value.code == FailureCode.EXECUTION_FAILED
            assert "contained no target text" in exc_info.value.message
