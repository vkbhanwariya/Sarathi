...
def test_native_extraction_statutory_continuation_preserves_resume_self(tmp_path: Path) -> None:
    """Verify that when OCR is needed and statutory option was requested, resume_self is set to True."""
    empty_pdf = tmp_path / "scanned_invoice.pdf"
    empty_pdf.write_bytes(
        b"%PDF-1.4\n1 0 obj<</Type/Catalog/Pages 2 0 R>>endobj\n2 0 obj<</Type/Pages/Count 1/Kids[3 0 R]>>endobj\n"
    ...
    inp = InputRef(
        input_id="inp-scanned-inv",
        source_path=empty_pdf,
        display_name="scanned_invoice.pdf",
        size_bytes=empty_pdf.stat().st_size,
    )
    req = Request(
        request_id="req-scanned-inv",
        requirement="read_native",
        inputs=(inp,),
        custom_options={"statutory": True},
    )
    ctx = ExecutionContext("run-scanned-inv", "req-scanned-inv", "t-3", "s-3")
