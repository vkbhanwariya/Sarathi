...
from sarathi.nabhi.pravaha import compute_input_hash
...
class TestPravahaFailureLifecycleAndQuarantine:
    ...
    def test_input_hash_represents_canonical_inputs_regression(
        ...
        hash1 = compute_input_hash(req1, cap, sample_context)
        hash2 = compute_input_hash(req2, cap, sample_context)
    ...
    def test_input_hash_deterministic_ordering_for_multi_inputs(
        ...
        hash_ab1 = compute_input_hash(req_ab, cap, sample_context)
        hash_ab2 = compute_input_hash(req_ab, cap, sample_context)
        hash_ba = compute_input_hash(req_ba, cap, sample_context)
    ...
    def test_input_hash_varies_with_custom_options(
        ...
        h1 = compute_input_hash(req1, cap, sample_context)
        h2 = compute_input_hash(req2, cap, sample_context)
        assert h1 != h2
    ...
    def test_typed_retry_executes_through_yantra_and_updates_lifecycle_truth(
        ...
        valid_input_hash = compute_input_hash(sample_request, cap, sample_context)
        record = QuarantineRecord(
    ...
    def test_execute_manual_retry_delivers_result_and_prior_result(
        self,
        kosh: Kosh,
        manthan: Manthan,
        yantra: Yantra,
        ...
        valid_input_hash = compute_input_hash(sample_request, cap, sample_context)
        record = QuarantineRecord(
            quarantine_id="quar-manual-01",
            input_hash=valid_input_hash,
            run_id=sample_context.run_id,
            request_id=sample_context.request_id,
            trace_id=sample_context.trace_id,
            capability_id="extract",
            plugin_id=c1_decl.plugin_id,
            failure_code=FailureCode.EXECUTION_FAILED,
            profile=sample_context.profile.value,
            attempt_count=0,
            max_retries=2,
            status=QuarantineStatus.QUARANTINED,
            created_at_utc="2026-09-01T00:00:00Z",
        ...
        prior_res = Result(data={"upstream_key": "val"})
        retry_act = LifecycleAction(
            action=LifecycleActionType.RETRY,
            item_id="quar-manual-01",
            request=sample_request,
            context=sample_context,
            prior_result=prior_res,
        )

        result, updated_rec = pravaha.execute_manual_retry(retry_act)

        assert result is not None
        assert cap.call_count == 1
        assert cap.received_prior_results == [prior_res]
        assert updated_rec.status is QuarantineStatus.RELEASED
        assert updated_rec.attempt_count == 1
    ...
    def test_retry_action_mismatched_request_id_fails_before_mutation(
        ...
        valid_hash = compute_input_hash(sample_request, cap, sample_context)
        record = QuarantineRecord(
    ...
    def test_retry_action_mismatched_kosh_declaration_fails_before_mutation(
        ...
        valid_hash = compute_input_hash(sample_request, tampered_cap, sample_context)
        record = QuarantineRecord(
    ...
    def test_pravaha_retry_cannot_bypass_kavacha_security_authorization(
        ...
        valid_hash = compute_input_hash(sample_request, denied_cap, sample_context)
        record = QuarantineRecord(
