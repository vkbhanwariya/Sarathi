...
def test_cache_key_includes_requirement(tmp_path: Path) -> None:
    """Verify that requests differing only in requirement produce distinct cache keys."""
    inp = InputRef("i1", tmp_path / "f.txt", "f.txt", 10)
    req1 = Request(request_id="r1", requirement="ocr", inputs=(inp,))
    req2 = Request(request_id="r1", requirement="native_extraction", inputs=(inp,))

    key1 = compute_cache_key(req1, "extraction_cap")
    key2 = compute_cache_key(req2, "extraction_cap")
    assert key1.key_hash != key2.key_hash


def test_hash_canonical_document_includes_source_input_id() -> None:
    """Verify CanonicalDocument with differing source_input_id produce distinct hashes."""
