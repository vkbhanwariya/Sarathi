...
class TestMukhaAuditCapabilityStatus:
    ...
    def test_build_inspector_view_projects_fallback_improvements(self) -> None:
        """Proves build_inspector_view projects NE-OCR fallback quality recovery records into typed views."""
        ...
        assert fb.fallback_engine == "NE-OCR"
        assert inspector.region_confidence[0].confidence_gain == 0.43
        assert inspector.region_confidence[0].fallback_engine == "NE-OCR"
        assert any(k == "Fallback Recoveries" for k, _ in inspector.system_facts)
