...
| :--- | :--- | :--- | :--- |
| **1. 🏦 Statement Consolidation & Reconciler** | Multi-bank statements (PDF, XLSX, CSV) | 3-Sheet Master Consolidated `.xlsx` + 27-col Parquet | Normalizes multi-month statements into canonical columns, resolving dates, signed balances, Indian banking INR ledger invariants (forex narration isolation), and isolated cheque vs reference numbers. Produces 3 sheets (`Transactions`, `Statements` reconciliation ledger, and `Exceptions` audit ledger), plus an enriched 27-column Decimal-preserving Parquet dataset. |
...
- **Currency & Forex Narration Isolation**: Statements default to Indian Rupee (`INR`); debit, credit, and balance columns on Indian bank accounts are strictly denominated in INR. Foreign currency mentions and exchange rates in transaction descriptions are isolated as descriptive narration and do not alter column amounts or statement currency.
