...
### ADR-004: Canonical Indian Bank Master Registry & Strict Bank Naming Invariant
- **Context**: When processing bank statements (especially Excel exports without registered YAML profiles), unconstrained name extraction or placeholder fallbacks ("Generic Bank") allowed arbitrary sheet headers, table titles, or account holder metadata to leak into statement and transaction bank names.
- **Decision**: Sarathi maintains an authoritative, zero-network catalog of all 158 RBI-recognized banking institutions in India (`src/sarathi/data/banks/banks_catalog.json`). All bank names emitted on `BankStatement` and `Transaction` instances must strictly match a canonical name from this registry. Identification follows a deterministic hierarchy (4-letter IFSC prefix $\rightarrow$ canonical exact/normalized name $\rightarrow$ colloquial aliases $\rightarrow$ document text scan). Statements whose institution cannot be verified against the catalog are strictly assigned `"Unknown Bank"` and emit a non-fatal `ValidationIssue(code="UNKNOWN_BANK", ...)` warning.
- **Consequences**: Eliminates false/random bank name leakage, ensures uniform display names across Parquet and Excel deliverables, and transparently audits unidentified statements in the Exceptions ledger.
