...
| Both Debit & Credit Blank / Zero | Inactive/blank debit and credit cells, or both cells containing `0.00` / `-` / blank. | Universal rule: `capability.py` normalizes `0` to `None`. If both debit and credit are `None` (or zero), row is flagged with issue `BOTH_AMOUNTS_BLANK` (`ValidationStatus.INVALID`) and automatically excluded from consolidated transactions. |
