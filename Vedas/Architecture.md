...
6. **High-Throughput I/O & Serialization Discipline**:
   ...
   - **Chunked Multi-Process PDF Vector Extraction (`ProcessPoolExecutor`)**: For PDFs $>4$ pages, PyMuPDF vector extraction is partitioned into page chunks dispatched across worker processes with isolated address spaces, eliminating Python GIL and Fitz context lock contention on multi-core processors. Preserves synchronous in-process execution for $\le 4$ pages with automatic serial fallback.
   ...
   - **Dual Financial Deliverables & Decimal-Precision I/O (`Polars` & `openpyxl`)**: Bank statement consolidation produces dual deliverables atomically committed via Nabhi: an enriched 27-column Apache Parquet dataset preserving exact Decimal precision via Polars `pl.Decimal(38, scale)` with microsecond read throughput, and a 3-sheet openpyxl workbook (`Transactions`, `Statements`, `Exceptions`) with native Excel dates and floats, formula injection protection (`s` type on descriptions and refs), and frozen auto-filter tables.
