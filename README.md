...
- **Bank Statement Processing & Multi-Account Consolidation**: Institutional schema mapping across 158 RBI-recognized banks, double-entry mathematical running balance reconciliation, multi-month cross-statement deduplication, automated UTR/IFSC syntax verification with heuristic OCR auto-repair (`utr_repair.py`), Master Account Directory generation (`List_of_Accounts.xlsx`) with unique account aggregation, continuous sequential transaction passbook (`Consolidated_Transactions.xlsx` with `TXN-0001`..`TXN-NNNN`), and forensic 32-column Parquet datasets (`Consolidated_Bank_Statement.parquet`).
...
- **Optional Cloud Providers**: Cloud OCR adapter for Mistral AI with proactive rate pacing, Retry-After header parsing, exponential backoff, and per-document legal isolation.
