...
class MukhaPresenter:
    ...
    def build_inspector_view(
        ...
        for pr in pramana_records:
            ...
            raw_eng = pr.attributes.get("fallback_engine") or "NE-OCR"
            fallback_eng = "NE-OCR" if raw_eng in ("ne_ocr", "NE-OCR") else str(raw_eng)
        ...
        if fallback_improvements:
            facts.append(("Fallback Recoveries", f"{len(fallback_improvements)} regions improved"))
