...
def build_document_preview(path_str: str) -> tuple[int, dict[str, Any]]:
    ...
    if ext in (".txt", ".csv", ".tsv", ".json", ".log", ".md", ".xml", ".html", ".py", ".yaml", ".yml"):
        try:
            ...
            logger.exception("Failed to read document preview: %s", target_file.name)
            return 500, {"ok": False, "error": "Failed to read document preview."}
    ...
    if ext in (".png", ".jpg", ".jpeg", ".bmp", ".webp", ".gif"):
        try:
            ...
            logger.exception("Failed to load image preview: %s", target_file.name)
            return 500, {"ok": False, "error": "Failed to load image preview."}
    ...
    if ext == ".pdf":
        try:
            ...
            logger.exception("Could not render PDF pages: %s", target_file.name)
            return 200, {
                "ok": True,
                "type": "pdf",
                "name": target_file.name,
    ...
    if ext in (".docx", ".doc"):
        try:
            ...
            logger.exception("Failed to read Word document preview: %s", target_file.name)
            return 500, {"ok": False, "error": "Failed to read Word document preview."}
...
def render_pdf_page(
    ...
    try:
        ...
        logger.exception("Failed to render PDF page %s: %s", page_number, target_file.name)
        return 500, {"ok": False, "error": "Failed to render PDF page."}
