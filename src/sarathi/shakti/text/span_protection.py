...
_NUM_RE: re.Pattern[str] = re.compile(r"(?:(?:Rs\.?|[₹$€£])\s?)?\b\d{1,3}(?:,\d{2,3})*(?:\.\d+)?%?")
...
class BaseSpanProtector:
    ...
    def restore(self, text: str, spans: Sequence[Any]) -> str:
        ...
        restored, _ = self.restore_with_validation(text, spans)
        return restored

    def restore_with_validation(self, text: str, spans: Sequence[Any]) -> tuple[str, list[dict[str, Any]]]:
        """Restore all protected spans with integrity validation.

        Returns tuple of (restored_text, issues_list).
        If any placeholder was deleted (count == 0) or duplicated (count > 1),
        an issue record is added describing the missing or duplicated span.
        """
