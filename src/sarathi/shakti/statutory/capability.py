    ...
    def execute(
        ...
        if prior_result is not None and prior_result.data is not None:
            if isinstance(prior_result.data, CanonicalDocument):
                ...
                text_items.append((prior_result.data.document_id, _extract_document_text(prior_result.data)))
                ...
                for d in prior_result.data:
                    if isinstance(d, CanonicalDocument):
                        ...
                        text_items.append((d.document_id, _extract_document_text(d)))
...
def _extract_document_text(doc: CanonicalDocument) -> str:
    """Extract full textual representation from CanonicalDocument, including tables and pages."""
    parts: list[str] = []
    if doc.text and doc.text.strip():
        parts.append(doc.text)
    for table in doc.tables:
        if table.headers:
            parts.append(" ".join(str(h) for h in table.headers if h is not None))
