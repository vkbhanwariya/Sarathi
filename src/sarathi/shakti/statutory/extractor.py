...
def extract_statutory_entities(text: str) -> StatutoryEntities:
    ...
    for word in words:
        ...
        if len(clean) == 8 and verify_din(clean):
            ...
            # if local context (within ~30 chars) contains 'DIN' or 'DIRECTOR'
            din_pattern = re.compile(
                rf"(?:(?:DIN|Director)[\s:.-]{{0,30}}{re.escape(clean)}|{re.escape(clean)}[\s:.-]{{0,30}}(?:DIN|Director))",
                re.IGNORECASE,
            )
            if din_pattern.search(text):
    ...
    if raw_identifiers["gstins"]:
        gst_list = raw_identifiers["gstins"]
        sup_gst: str | None = None
        buy_gst: str | None = None

        if len(gst_list) == 1:
            sup_gst = gst_list[0]
        else:
            buyer_kw = re.compile(r"\b(?:buyer|billed\s+to|bill\s+to|recipient|customer|consignee)\b", re.IGNORECASE)
            supplier_kw = re.compile(r"\b(?:supplier|seller|vendor|sold\s+by|from|consignor|provider)\b", re.IGNORECASE)
            roles: dict[str, str] = {}
    ...
    if gst_meta and gst_meta.supplier_gstin:
        ...
        if gst_meta.invoice_number and gst_meta.invoice_date:
            confidence = 0.95
        elif gst_meta.invoice_number or gst_meta.invoice_date:
            confidence = 0.90
        else:
            confidence = 0.85
        ...
        has_tax_context = bool(
            it_meta.ack_number
            or it_meta.assessment_year
            or re.search(
                r"\b(?:INCOME\s*TAX|ITR|TAX\s*RETURN|ACKNOWLEDGEMENT|FORM\s*16|FORM\s*26AS)\b",
                text,
                re.IGNORECASE,
            )
        )
        if has_tax_context:
            if "FORM 16" in text.upper():
                doc_type = StatutoryDocumentType.FORM_16
            else:
                doc_type = StatutoryDocumentType.INCOME_TAX_ACK

            if it_meta.ack_number and it_meta.pan:
                confidence = 0.95
            elif it_meta.ack_number or (it_meta.pan and it_meta.assessment_year):
                confidence = 0.92
            else:
                confidence = 0.75
        else:
