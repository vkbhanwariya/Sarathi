...
def read_xls_legacy(
    ...
    """Extract all sheets from legacy BIFF .xls using python-calamine with xlrd fallback."""
    tables: list[TableData] = []
    provenances: list[ProvenanceRecord] = []
    warnings: list[WarningRecord] = []
    composite_lines: list[str] = []
    ...
    try:
        ...
        for sheet_name in wb.sheet_names:
            ...
            composite_lines.append(f"--- Sheet: {sheet_name} ---")
            if headers:
                composite_lines.append(" | ".join(str(h) for h in headers if str(h).strip()))
            for row in data_rows:
                row_str = " | ".join(str(c) for c in row if c is not None and str(c).strip())
                if row_str:
                    composite_lines.append(row_str)
    except Exception:
        # Fallback to xlrd for bare BIFF streams or legacy structures unsupported by Calamine
        tables.clear()
        provenances.clear()
        composite_lines.clear()
        warnings.append(
            WarningRecord(
                code="CALAMINE_XLS_FALLBACK",
                message="python-calamine failed on legacy .xls workbook, fell back to xlrd reader.",
                stage=STAGE_NAME,
            )
        )
