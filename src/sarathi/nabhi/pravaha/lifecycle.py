...
def execute_manual_retry(
    ...
    *,
    ...
    """Execute a manual retry on a quarantined item and return (Result, QuarantineRecord)."""
    ...
    if action.action is not LifecycleActionType.RETRY:
        raise DoshError(
            code=FailureCode.VALIDATION_FAILED,
            message=f"Action '{action.action}' is not a RETRY action.",
        )
    if quarantine_store is None:
        raise DoshError(
            code=FailureCode.INVALID_CONFIGURATION,
    ...
    effective_prior = prior_result if prior_result is not None else action.prior_result
    ...
    return execute_retry_attempt(
        cap=cap,
        request=effective_req,
        context=effective_ctx,
        record=existing,
        prior_result=effective_prior,
...
def apply_lifecycle_action(
    ...
    match action.action:
        ...
        case LifecycleActionType.RETRY:
            _, updated_rec = execute_manual_retry(
                action=action,
                request=effective_req,
                context=effective_ctx,
                prior_result=prior_result or action.prior_result,
                quarantine_store=quarantine_store,
                retry_policy=retry_policy,
                yantra=yantra,
                darpana=darpana,
